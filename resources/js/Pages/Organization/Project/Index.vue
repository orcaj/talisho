<template>
    <v-container>
        <organization-header :organization="organization" :company="company">
            <inertia-link
                v-if="$page.auth.user.can.create.projects"
                class="no-underline"
                :href="
            ...
            <v-col cols="12">
                ...
                >
                    <template v-slot:header.status>
                        <v-tooltip
                            v-if="$page.auth.user.can.view.disciplines"
                    ...
                    <template v-slot:item.status="{ item }">
                        <v-tooltip content-class="summary-tooltip" bottom>
                            <template v-slot:activator="{ on }">
                                <status-light
                                    :should-show-tooltip="
                                        $page.auth.user.can.view.disciplines ||
                                            $page.auth.user.id ===
                                                item.disciplines[0].lead.id
                                    "
                                    :on-hover="on"
                                    :color="
                                        moreThanOneDiscipline(item)
                                            ? item.status.color
                                            : item.disciplines[0].status.color
                                    "
                                />
                            </template>
                            <project-summary-tooltip
                                :summary-data="
                                    ...
                                    moreThanOneDiscipline(item)
                                        ? item.summaryData
                                        : item.disciplines[0].summaryData
                                "
                            />
                        </v-tooltip>
                    </template>
                    ...
                    <template v-slot:item.notifications="{ item }">
                        <notifications-dropdown
                            :base-notifications="item.notifications"
                            :project-id="item.id"
                        />
                    </template>
...
export default {
    ...
    data(vm) {
        return {
            tab: this.$page.auth.user.can.view.disciplines
                ? "manager"
                : "guest",
            projects: this.$page.auth.user.can.view.disciplines
                ? this.mprojects
                : this.gprojects,
            headers: [
                {
    ...
    methods: {
        showProjectSummary(project, disciplineId = null) {
            ...
            this.$inertia.visit(
                this.route("organizations.projects.documentation.index", {
